...
export function ChatFlow() {
  ...
  const carregarTudo = async () => {
    ...
    try {
        ...
        // 🔥 BUSCA O ORDER BUMP DO BOT (SEM DEPENDER DO API.JS PARA EVITAR ERROS)
        try {
            const API_BASE = import.meta.env.VITE_API_URL?.replace(/\/$/, '') || 'http://localhost:8000';
            ...
            // Tenta rota 2 se a primeira falhar
            if (!obRes || !obRes.ok) {
  ...
  const getPlanosPrincipais = () => {
      ...
      return availablePlans.filter(p => {
          ...
          const isExtra = p.tipo === 'order_bump' || p.tipo === 'upsell' || p.tipo === 'downsell' || 
                          p.is_order_bump === true || p.is_upsell === true || p.is_downsell === true;
          return isAtivo && !isExtra;
      });
  };
  ...
  const handleSimNext = (currentMsgId, nextIndex, actionBtn = null) => {
      ...
      setTimeout(() => {
          ...
          if (actionBtn && actionBtn.type === 'link') {
              Swal.fire({
                  title: 'Link Clicado!', 
                  text: `O bot abriria: ${actionBtn.url || actionBtn.text}`, 
                  icon: 'info', 
                  background: '#111', 
                  color: '#fff'
              });
  ...
  const showPixMessage = (plan, acceptedOB) => {
      ...
      // 🔥 O HTML idêntico ao CTA do Telegram
      const fakePixHtml = `<div class="pix-mock-wrapper">
          <div class="pix-mock-code">${fakePix}</div>
          <div class="pix-mock-action">
  ...
  // --- GERENCIADOR DE BOTÕES MENSAGEM 1 ---
  ...
  // --- GERENCIADOR DE BOTÕES DA MENSAGEM 2 ---
  ...
  const handleOpenEditModal = (step) => {
    setEditingStep(step);
    setModalData({
      msg_texto: step.msg_texto || '', msg_media: step.msg_media || '', btn_texto: step.btn_texto || 'Próximo ▶️', autodestruir: step.autodestruir || false, mostrar_botao: step.mostrar_botao !== false, delay_seconds: step.delay_seconds || 0
    });
    setShowModal(true);
  };
                    ...
                    <div className="messages-area" style={{ overflowY: 'auto', display: 'flex', flexDirection: 'column', paddingBottom: '20px' }}>
                        {simMessages.map((msg, index) => (
                            <div key={msg.id + index} className="msg-bubble bot fade-in-up" style={{ alignSelf: 'flex-start', marginBottom: '15px' }}>
                                ...
                                {/* TEXTO RENDERIZADO (HTML + EMOJIS REAIS + PIX) */}
                                {msg.text && (
                        ...
                        {isAudioUrl(flow.media_url) && (
                            <div style={{
                                ...
                                <p style={{color: '#eab308', fontSize: '0.85rem', margin: 0}}>
                                    🎙️ <strong>Modo Áudio Ativo</strong> — Botões de ação ficam desabilitados. O áudio será enviado como voice note nativo.
                                </p>
                            </div>
                        )}
                                    ...
                                    <div className="fade-in-up" style={{marginLeft: 25, padding: '15px', background: 'rgba(255,255,255,0.02)', borderRadius: '8px', border: '1px solid #333'}}>
                                        <div style={{marginBottom: 20}}>
                                            ...
                                            <button 
                                                ...
                                                style={{
                                                    marginTop: 10, background: '#c333ff', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: '6px', cursor: 'pointer', fontSize: '0.9rem', fontWeight: 600
                                                }}
                                            >
                                                + Adicionar
                                            </button>
                                            ...
                                            <div>
                                                ...
                                                {flow.buttons_config.map((btn, index) => (
                                                    <div 
                                                        key={btn.id} 
                                                        style={{
                                                            background: '#111', padding: '10px', borderRadius: '6px', marginBottom: '8px', border: '1px solid #333', display: 'flex', alignItems: 'center', justifyContent: 'space-between'
                                                        }}
                                                    >
                                                        <div style={{flex: 1}}>
                                                        ...
                                                        <div style={{display: 'flex', gap: '5px'}}>
                                                            <button onClick={() => handleMoveButton(index, 'up')} disabled={index === 0} style={{background: '#222', border: '1px solid #333', padding: '4px', borderRadius: '4px', cursor: 'pointer'}}>
                                                                <ChevronUp size={16} color={index === 0 ? '#444' : '#fff'} />
                                                            </button>
                                                            <button onClick={() => handleMoveButton(index, 'down')} disabled={index === flow.buttons_config.length - 1} style={{background: '#222', border: '1px solid #333', padding: '4px', borderRadius: '4px', cursor: 'pointer'}}>
                                                                <ChevronDown size={16} color={index === flow.buttons_config.length - 1 ? '#444' : '#fff'} />
                                                            </button>
                                                            <button onClick={() => handleRemoveButton(index)} style={{background: '#ef4444', border: 'none', padding: '4px', borderRadius: '4px', cursor: 'pointer'}}>
                                ...
                                <div style={{marginTop: 15, padding: '15px', background: 'rgba(255,255,255,0.02)', borderRadius: '8px', border: '1px solid #333'}}>
                                    ...
                                    <div style={{marginBottom: 15}}>
                                        ...
                                        <button 
                                            ...
                                            style={{
                                                marginTop: 10, background: '#10b981', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: '6px', cursor: 'pointer', fontSize: '0.9rem', fontWeight: 600
                                            }}
                                        >
                                            + Adicionar
                                        </button>
                                        ...
                                        <div>
                                            ...
                                            {flow.buttons_config_2.map((btn, index) => (
                                                ...
                                                >
                                                    ...
                                                    <div style={{display: 'flex', gap: '5px'}}>
                                                        <button onClick={() => handleMoveButton2(index, 'up')} disabled={index === 0} style={{background: '#222', border: '1px solid #333', padding: '4px', borderRadius: '4px', cursor: 'pointer'}}>
                                                            <ChevronUp size={16} color={index === 0 ? '#444' : '#fff'} />
                                                        </button>
                                                        <button onClick={() => handleMoveButton2(index, 'down')} disabled={index === flow.buttons_config_2.length - 1} style={{background: '#222', border: '1px solid #333', padding: '4px', borderRadius: '4px', cursor: 'pointer'}}>
                                                            <ChevronDown size={16} color={index === flow.buttons_config_2.length - 1 ? '#444' : '#fff'} />
                                                        </button>
                                                        <button onClick={() => handleRemoveButton2(index)} style={{background: '#ef4444', border: 'none', padding: '4px', borderRadius: '4px', cursor: 'pointer'}}>
                                                            <Trash2 size={16} color="#fff" />
                                                        </button>
                                                    </div>
                                                </div>
            ...
            <div className="modal-content">
                ...
                <div className="modal-body">
                    ...
                    {isAudioUrl(modalData.msg_media) && (
                        <div style={{
                            background: 'rgba(234, 179, 8, 0.1)', border: '1px solid rgba(234, 179, 8, 0.3)', borderRadius: '8px', padding: '12px 15px', marginTop: '10px', marginBottom: '10px'
                        }}>
                            <p style={{color: '#eab308', fontSize: '0.85rem', margin: 0}}>
                                🎙️ <strong>Modo Áudio</strong> — Botões ficam desabilitados. O áudio será enviado sozinho como voice note.
                            </p>
                        </div>
                    )}
                        ...
                        {!isAudioUrl(modalData.msg_media) ? (
                            <>
                                <label className="checkbox-label">
                                    <input type="checkbox" checked={modalData.mostrar_botao} onChange={e => setModalData({...modalData, mostrar_botao: e.target.checked})} /> 
                                    Mostrar botão "Próximo"?
                                </label>
                                {modalData.mostrar_botao ? (
                                    <Input label="Texto do Botão" value={modalData.btn_texto} onChange={e => setModalData({...modalData, btn_texto: e.target.value})} />
                                ) : (
                                    <div className="delay-input-wrapper">
                                        <Input label="Intervalo (s)" type="number" value={modalData.delay_seconds} onChange={e => setModalData({...modalData, delay_seconds: parseInt(e.target.value) || 0})} icon={<Clock size={16}/>} />
                                    </div>
                                )}
                            </>
                        ) : (
                    ...
                    <div className="toggle-wrapper modal-toggle">
                        ...
                        <div className={`custom-toggle ${modalData.autodestruir ? 'active' : ''}`} onClick={() => setModalData({...modalData, autodestruir: !modalData.autodestruir})}>
                            <div className="toggle-handle"></div>
                        </div>
                    </div>
                    <div className="modal-actions">
                        <button className="btn-cancel" onClick={() => setShowModal(false)}>Cancelar</button>
                        <button className="btn-save" onClick={handleSaveStep}>Salvar</button>
                    </div>
                </div>
            </div>
        </div>
